        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                while (!Console.KeyAvailable)
                {
                    ...
                    MemoryFloat HeatA = MemoryMap.Instance.GetFloat(1, MemoryType.Input);
                    MemoryBit HeatAv = MemoryMap.Instance.GetBit(9, MemoryType.Output);

                    MemoryFloat HeatD = MemoryMap.Instance.GetFloat(13, MemoryType.Input);
                    MemoryBit HeatDv = MemoryMap.Instance.GetBit(44, MemoryType.Output);

                    MemoryFloat HeatE = MemoryMap.Instance.GetFloat(25, MemoryType.Input);
                    MemoryBit HeatEv = MemoryMap.Instance.GetBit(57, MemoryType.Output);

                    MemoryFloat HeatN = MemoryMap.Instance.GetFloat(128, MemoryType.Input);
                    MemoryBit HeatNv = MemoryMap.Instance.GetBit(177, MemoryType.Output);

                    MemoryFloat HeatG = MemoryMap.Instance.GetFloat(46, MemoryType.Input);
                    MemoryBit HeatG1v = MemoryMap.Instance.GetBit(86, MemoryType.Output);
                    MemoryBit HeatG2v = MemoryMap.Instance.GetBit(87, MemoryType.Output);

                    MemoryFloat HeatH = MemoryMap.Instance.GetFloat(58, MemoryType.Input);
                    MemoryBit HeatHv = MemoryMap.Instance.GetBit(100, MemoryType.Output);

                    MemoryFloat HeatI = MemoryMap.Instance.GetFloat(69, MemoryType.Input);
                    MemoryBit HeatIv = MemoryMap.Instance.GetBit(112, MemoryType.Output);

                    MemoryFloat HeatJ = MemoryMap.Instance.GetFloat(81, MemoryType.Input);
                    MemoryBit HeatJv = MemoryMap.Instance.GetBit(125, MemoryType.Output);

                    MemoryFloat HeatK = MemoryMap.Instance.GetFloat(92, MemoryType.Input);
                    MemoryBit HeatKv = MemoryMap.Instance.GetBit(136, MemoryType.Output);

                    MemoryFloat HeatM = MemoryMap.Instance.GetFloat(116, MemoryType.Input);
                    MemoryBit HeatMv = MemoryMap.Instance.GetBit(162, MemoryType.Output);

                    MemoryFloat HeatL = MemoryMap.Instance.GetFloat(104, MemoryType.Input);
                    MemoryBit HeatLv = MemoryMap.Instance.GetBit(149, MemoryType.Output);

                    MemoryBit B1 = MemoryMap.Instance.GetBit(274, MemoryType.Input);

                    MemoryBit DoorA1 = MemoryMap.Instance.GetBit(13, MemoryType.Input);
                    MemoryBit LightO1 = MemoryMap.Instance.GetBit(187, MemoryType.Output);
                    MemoryBit LightO2 = MemoryMap.Instance.GetBit(188, MemoryType.Output);
                    MemoryBit LightOPool = MemoryMap.Instance.GetBit(189, MemoryType.Output);
                    MemoryBit LightOGarden = MemoryMap.Instance.GetBit(190, MemoryType.Output);
                    //MemoryFloat BrightO = MemoryMap.Instance.GetFloat(139, MemoryType.Input);
                    if(tiempo.Hour <= 5 || tiempo.Hour >= 19)
                    {
                        LightOGarden.Value = true;
                    }
                    else
                    {
                        LightOGarden.Value = false;
                    }
                    if (DoorA1.Value == false && (tiempo.Hour <= 5 || tiempo.Hour >= 19))
                    {
                        LightO1.Value = true;
                        LightO2.Value = true;
                        LightOPool.Value = true;
                    } else
                    {
                        LightO1.Value = false;
                        LightO2.Value = false;
                        LightOPool.Value = false;
                    }
                    ...
                    if (HeatA.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatAv.Value = true;
                    }
                    if (HeatA.Value > 18)
                    ...
                    if (HeatD.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatDv.Value = true;
                    }
                    if (HeatD.Value > 18)
                    ...
                    if (HeatE.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatEv.Value = true;
                    }
                    if (HeatE.Value > 18)
                    ...
                    if (HeatN.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatNv.Value = true;
                    }
                    if (HeatN.Value > 18)
                    ...
                    if (HeatG.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatG1v.Value = true;
                        HeatG2v.Value = true;
                    }
                    ...
                    if (HeatH.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatHv.Value = true;
                    }
                    if (HeatH.Value > 18)
                    ...
                    if (HeatI.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatIv.Value = true;
                    }
                    if (HeatI.Value > 18)
                    ...
                    if (HeatJ.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatJv.Value = true;
                    }
                    if (HeatJ.Value > 18)
                    ...
                    if (HeatK.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatKv.Value = true;
                    }
                    if (HeatK.Value > 18)
                    ...
                    if (HeatL.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatLv.Value = true;
                    }
                    if (HeatL.Value > 18)
                    ...
                    if (HeatM.Value <= 18 && (tiempo.Hour <= 8 || tiempo.Hour >= 20))
                    {
                        HeatMv.Value = true;
                    }
                    if (HeatM.Value > 18)
                    ...
                    //****************************************************************************
                    if (tiempo.Hour >= 6 && tiempo.Hour <= 18 && rollA1up.Value == false && b1== false)
                    {
                    ...
                    if ((tiempo.Hour <= 5 || tiempo.Hour >= 19) && rollA1down.Value == false && b1== false)
                    {
